...
<body>
        ...
        <table class="table">
         ...
         <tbody>
            <?php foreach($questions as $question) { ?>
              <form action="./services/delete_question.php" method="POST">
  ...
  <script>
    const deleteQuestion = async (questionId) => {
      ...
      const res = await fetch(`http://localhost:8080/admin/services/delete_question.php?id=${questionId}`, { method: 'DELETE' });
      if (res.status === 204) {
        alert('削除に成功しました')
